...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
            ...
            c2=args[1]
            args = [*args]
        elif m in [MambanoCrossfusion2,MambaLearnablefusion2,MambanoCrossLearnablefusion2,MambanoCrossfusion,Mambafusion2,CrossMambaFusionBlock, ConcatMambaFusionBlock,noCrossMambaFusionBlock, CrossMambaFusionreturnBlock,Mambafusion]:
            c2 = ch[f[0]]

        elif m is Mambafusionall:
            c2 = ch[f[0]]
            args = [c2, *args[1:]]
        else:
            c2 = ch[f]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./yolov5l_CrossmambafusionReturn_FLIR.yaml', help='model.yaml')
    ...
    for i in range(8):
        ...
        ir=[]
        for j in range(3):
            rgb.append(torch.Tensor(np.random.randint(0,256,(640,640))))
            ir.append(torch.Tensor(np.random.randint(0,256,(640,640))))
        input_rgb1.append(np.stack((rgb[0],rgb[1],rgb[2])))
        input_ir1.append(np.stack((ir[0], ir[1], ir[2])))
    input_rgb=np.stack((input_rgb1[0],input_rgb1[1]))
    input_ir=np.stack((input_ir1[0],input_ir1[1]))
    input_rgb =torch.Tensor(input_rgb)
    input_ir = torch.Tensor(input_ir)
