...
class YOLOV5GradCAMPP(YOLOV5GradCAM):
    ...
    def forward(self, mg_vis, img_ir, class_idx=True):
        saliency_maps = []
        b, c, h, w = img_ir.size()
        tic = time.time()
        preds, logits = self.model(mg_vis, img_ir)
        print("[INFO] model-forward took: ", round(time.time() - tic, 4), 'seconds')
        for logit, cls, cls_name in zip(logits[0], preds[1][0], preds[2][0]):
            ...
            # 获取梯度
            score.backward(retain_graph=True)
            print(f"[INFO] {cls_name}, model - backwardtook: ", round(time.time() - tic, 4), 'seconds')
