...
target = ['model__30__cv3__act',  'model__33__cv3__act',  'model__36__cv3__act']
# fusion_target_baseline=[]
...
#NiN
# target = ['model_20_act', 'model_21_act','model_22_act']
# mamba
# target = ['model__20__channel_attn_mamba__op__act', 'model__21__channel_attn_mamba__op__act','model__22__channel_attn_mamba__op__act']
# target = ['model__20__conv1x1_out__act', 'model__21__conv1x1_out__act','model__22__conv1x1_out__act']
# Arguments
parser = argparse.ArgumentParser()
parser.add_argument('--model-path', type=str, default="runs/train/bestmodel/yolov5l_Cross_Concat_FLIR-align-3class-83/weights/best.pt", help='Path to the model')
parser.add_argument('--source1', type=str, default='visualization/example/visible', help='source')  # file/folder, 0 for webcam
parser.add_argument('--source2', type=str, default='visualization/example/infrared', help='source')  # file/folder, 0 for webcam
parser.add_argument('--output-dir', type=str, default='visualization/FLIR-align-3class/bicycle/best', help='output dir')
...
parser.add_argument('--choose_names', type=str, default='bicycle', help='class to show')
#'person, car, bicycle'
...
def put_text_box(bbox, cls_name, res_img):
    ...
    cv2.imwrite('temp.jpg', (res_img ).astype(np.uint8))
    return res_img
    ...
    if args.method == 'gradcam':
        for layer in args.target_layer:
            ...
            for i, mask in enumerate(masks):
                if class_names[0][i]==args.choose_names:
                    bbox = boxes[0][i]
                    mask = mask.squeeze(0).mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).detach().cpu().numpy().astype(np.uint8)
                    heatmap = cv2.applyColorMap(mask, cv2.COLORMAP_JET)
                    ret,heatmap=cv2.threshold(heatmap,220,128,3)
                    n_heatmat = (heatmap / 255).astype(np.float32)
                    # n_heatmat=(Box.fill_outer_box(heatmap, bbox, value=0) / 255).astype(np.float32)
                    heat.append(n_heatmat)
                    # res_img, heat_map = get_res_img(bbox, mask, res_img)
                    # res_img = put_text_box(bbox, class_names, res_img)  # plot the bboxes
            ...
            if(len(heat) != 0):
                ...
                res_img = cv2.addWeighted(res_img, 0.5, heat_avg, 0.5, 0)
            res_img = (res_img / res_img.max())
        ...
        output_path = f'{args.output_dir}/{img_name}'
        img_all = images[0].astype(np.float32)
        for img in images[1:]:
            img_all += img
...
if __name__ == '__main__':
    if os.path.isdir(args.source1):
        ...
        img_vis_list.sort()
        for item in img_vis_list[4:5]:
            img_vis_path = os.path.join(args.source1 ,item)
            if args.source1 == 'visualization/example/visible':
        ...
        img_vis_path=args.source1
        img_ir_path=args.source2
        main(img_vis_path, img_ir_path)